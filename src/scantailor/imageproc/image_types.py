...
Type Conventions:
    - BinaryImage: uint8 array with values 0 (black/background)
      or 255 (white/foreground)
    - GrayImage: uint8 array with grayscale values 0-255
    - ColorImage: uint8 array with shape (H, W, 3) for BGR
      color images (OpenCV convention)
...
def is_binary(image: NDArray[np.uint8]) -> bool:
    ...
    return bool(len(unique) <= 2 and np.all((unique == 0) | (unique == 255)))
